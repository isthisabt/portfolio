...
video_id <- "insert-video-id-here> it-is-usually-the-string-right-after-?-sign-in-video-link"
...
repeat {
  ...
  next_page_token <- response$nextPageToken ###This will update the next_page_token based on the page token we get in response
}
...
#----Calculate the average sentiment for each unique sentence_i-----IGNORE
#----average_sentiment <- sentiment_scores %>%                  ----THIS
 # ----group_by(element_id) %>%                                   --SECTION
...
file_path <- "ouput/file/location/here/file_name_here.xlsx"
...
file_path <- "ouput/file/location/here/file_name_here_negative.xlsx"
